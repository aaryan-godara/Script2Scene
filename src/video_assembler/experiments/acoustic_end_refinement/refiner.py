"""Deterministic acoustic speech-end refinement (isolated experiment).

Refines ONLY speech_end from the audio signal. speech_start and scene matching
are never touched. All parameters are configurable; no GT values are read while
computing timestamps -- the algorithm consumes only the audio signal, the existing
alignment, and the next-scene boundary.
"""
...
class AcousticBoundaryRefiner:
    """Configurable, deterministic acoustic-tail end refiner."""
    ...
    def _db(self, value: float) -> float:
        return 20.0 * np.log10(value / max(self._threshold, self.config.energy_floor))
    ...
    def refine(self, alignments: List[Dict]) -> Tuple[List[SceneRefinement], RefinementStats]:
        ...
        for i, a in enumerate(alignments):
            ...
            # safety clamps (never shorten below raw, never cross boundaries)
